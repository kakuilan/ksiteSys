 ...
 * Date: 2018/1/27
 * Time: 21:32
 * Desc: -IP地址转换服务类,来源https://github.com/lionsoul2014/ip2region
 */
...
class Ip2RegionService extends ServiceBase {
    ...
    public function __construct(array $vars = []) {
        ...
        $this->dbFile = DATADIR . 'ip2region.db';
    }
    ...
    public function getCityName($ip) {
        $res = '';
        if(!empty($ip)) {
            $arr = $this->btreeSearch($ip);
            if(is_array($arr) && !empty($arr)) {
                $arr = explode('|', $arr['region']);
                $res = $arr[3] ?? '';
            }
        }
