...
require.config({
    ...
    paths: {
        ...
        'md5': '../../statics/js/md5.min',
        'lkkTabMenu': '../../statics/js/lkkTabMenu',
        'lkkFunc': '../../statics/js/lkkFunc',
        'emojione': '../libs/emojione/emojione.min',
    },
    // shim依赖配置
    shim: {
        'bootstrap': ['jquery'],
                        ...
                        if (ret.hasOwnProperty("code")) {
                            ...
                            if (status) {
                                if (typeof success == 'function') {
                                    var onAfterResult = success.call(undefined, data, ret);
                                    if (!onAfterResult) {
                                        return false;
                                    }
                                }
...
define('table',['jquery', 'bootstrap', 'moment', 'moment/locale/zh-cn', 'bootstrap-table', 'bootstrap-table-lang', 'bootstrap-table-mobile', 'bootstrap-table-export', 'bootstrap-table-commonsearch', 'bootstrap-table-template'], function ($, undefined, Moment) {
    var Table = {
        ...
        api: {
            ...
            // 批量操作请求
            multi: function (action, ids, table, element, params) {
                var options = table.bootstrapTable('getOptions');
                var data = element ? $(element).data() : {};
                var url = typeof data.url !== "undefined" ? data.url : (action == "del" ? options.extend.del_url : options.extend.multi_url);
                url = url + (url.match(/(\?|&)+/) ? "&ids=" : "?ids=") + ($.isArray(ids) ? ids.join(",") : ids);
                var params = params ? params : typeof data.params !== "undefined" ? (typeof data.params == 'object' ? $.param(data.params) : data.params) : '';
                        ...
                        init: {
                            ...
                            FileUploaded: function (up, file, info) {
                                ...
                                try {
                                    ...
                                    if (ret.hasOwnProperty('code')) {
                                        ret.data = ret.data ? ret.data : ret;
                                        var data = ret.hasOwnProperty("data") && ret.data != "" ? ret.data : null;
                                        var msg = ret.hasOwnProperty("msg") && ret.msg != "" ? ret.msg : "";
                                        var status = ret.hasOwnProperty("status") && ret.status !== "" ? ret.status : false;
                                        if (status) {
